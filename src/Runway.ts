...
export default class Runway {
	...
	public toString(): string {
		return [
			...
			Math.round(this.displacedThreshold * 100) / 100,
			Math.round(this.reverseDisplacedThreshold * 100) / 100,
			this.elevation,
			this.glideslope,
			Math.round(this.localizer * 100) / 100,
			this.reverseGlideslope,
			Math.round(this.reverseLocalizer * 100) / 100,
